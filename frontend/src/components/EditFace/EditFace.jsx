...
const EditFace = () => {
  ...
  const [isWebcamOn, setIsWebcamOn] = useState(() => true); // Set initial state conditionally
  const [imageName, setImageName] = useState('');
  const [imageCount, setImageCount] = useState(0);
  const [capturedImages, setCapturedImages] = useState([]);

  const [maxImg, setMaxImg] = useState();   //Maximum allowed images to be captured.
  const { user_id, user_name, imgCount } = useParams(); //user_id and imgcount from the URL.
  ...
  const captureImage = async (event) => {
    ...
    const count_image = 4 - imgCount
    setMaxImg(count_image);

    console.log(user_id, user_name, imgCount);

    const newImageCount = imageCount + 1;
    console.log(newImageCount);

    if (newImageCount > maxImg) { // Check if the user tries to capture more than the Specified images in the "maxImg".
      setModalMessage(`You have already captured the Maximum ${maxImg} images.`);
  ...
  const uploadImages = async () => {
    try {
      ...
      if (capturedImages.length !== maxImg) {
        console.log("Number of captured images:", capturedImages.length);
        setModalMessage(`Please select Exactly ${maxImg} images.`);
        setShowMessageModal(true);
        return;
      }
      ...
      capturedImages.forEach((imageSrc, index) => {
        ...
        formData.append('images', blob, `${user_name}${index + 1}.jpg`);
      });

      formData.append('user_id', user_id);
      formData.append('user_name', user_name);

      const response = await fetch('http://localhost:5000/upload_edit', {
        method: 'POST',
        body: formData,
      });
      ...
      if (data.access === 'Granted') {
        ...
        setModalMessage(`Your images were uploaded successfully to Folder name: ${data.folderName}`);
        setShowMessageModal(true);
